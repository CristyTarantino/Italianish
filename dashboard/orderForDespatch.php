...
?>
<?php 
	if(isset($_POST['confirm']) && $_POST['orderNumber'] > 0){
		$orderNumber = $_POST['orderNumber'];
		
		// send confirmation mail to the customer
		$to = $row_rsCustomer['customerEmail'];
		$subject = 'Recipesjack - Confirmation of Order';
		$body = 'Dear '.$row_rsCustomer['customerFirstName'].' '.
		$row_rsCustomer['customerLastName'].',
		Thank you for ordering Recipesjack recipes.
		This mail is to confirm that your order No.'.$orderNumber.' has been despatched.
		Thank you and best wishes
		M Cristina Tarantino';
		$headers = "From: noreply@recipesjack.com";
		mail ($to, $subject, $body, $headers);
		
		// update the order status to paid and return to Sales Dept.
		mysql_select_db($database_mctarant_assignment_2012, $mctarant_assignment_2012);
		$sql = "UPDATE orders
...
<title>Order awaiting Despatch</title>
</head>

<body>
<div id="container">
        <div class="dbdReport">
        <h1>Order awaiting Despatch</h1>
...
?>
    ...
    <form action="" method="post">
      <input name="orderNumber" type="hidden" value="<?php echo $row_rsOrder['orderNumber']?>">
      <input class="button" id="confirm" name="confirm" type="submit" value="Order Despatched">
