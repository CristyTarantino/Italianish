...
<title>Orders for Fulfilment and Despatch</title>
</head>

<body>
<div id="container">
        <div class="dbdReport">
        <h1>Orders for Fulfilment and Despatch</h1>
        <p id="logout"><small><a href="../login/logout.php">Logout</a></small></p>
        <p>This report shows orders received and with payment verified. When the order has been despatched click on the link to move the order into Orders Despatched (sends order confirmation to customer).</p>
        <?php if($totalRows_rsOrders <= 0) { ?> 
		<blockquote>There are no orders in this category</blockquote> 
		<?php } else do { ?>
        <div class="orderItem"> 
        <span class="orderNumber">
        <a href="orderForDespatch.php?orderID=<?php echo $row_rsOrders['orderNumber']; ?>"> 
